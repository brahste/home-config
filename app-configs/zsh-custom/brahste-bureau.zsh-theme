...
_PATH="%{%B$FG[035]%}%B%(4~|%-1~/../%2~|%3~)%{$reset_color%}"
...
env_custom_line() {
    if ! [[ -z "${DIRENV_FILE}" ]]; then
      ...
      echo "%{$FG[027]%}${env_specifier}%{$reset_color%}"
    fi
}

# Use the top line below to include the username
#_1LEFT="$_USERNAME $_PATH"
_1LEFT="$_PATH"
_1RIGHT="$(env_custom_line)"
